...
def spectral_clustering_division(E, geoms, split_type="threshold"):
    ...
    nrlz = np.array([1., 1., 1.]) #np.array([640., 480., 1e2])
    ngeoms = geoms.astype('float64') / nrlz[np.newaxis, :]
...
def build_sym_geom_adjacency(geoms, max_gnn=100):
    ...
    for n_neighbors in range(min_gnn, max_gnn + 1):
        ...
        C = C + C.T
        C.data[:] = 1
        n_comp, _ = sparse.csgraph.connected_components(C) # sparse.cs_graph_components(C)
        if n_comp == 1:
